#!/usr/bin/env python
# -*- coding: utf-8 -*-
'''A simple pyspark script to split total data in a parquet-backed dataframe
   Return train, val, test dataframe  
Usage:
    $ spark-submit data_split_sj.py hdfs:/user/sjc433/is_read_30.parquet
'''
...
#     df_more30 = df.where(df.num_interaction > 30)
    user_id_ls = interaction.select('user_id').distinct()
...
#     interaction.write.format("parquet").mode("overwrite").save("hdfs:/user/sjc433/filtered.parquet")

    # split 60%, 20%, 20% of downsample based on distinct user id
    seed=42
    train_user, val_user, test_user = interaction\
        .select("user_id")\
        .distinct()\
    ...
    train = train_user.join(interaction, 'user_id', 'inner')
    val = val_user.join(interaction, 'user_id', 'inner')
    test = test_user.join(interaction, 'user_id', 'inner')
...
#     print("number of distinct user ids of test data: ", test_left.select("user_id").distinct().count())
    ...
    train_df.write.format("parquet").mode("overwrite").save("hdfs:/user/sjc433/train_total.parquet")
    print("train df saved")
    val_left.write.format("parquet").mode("overwrite").save("hdfs:/user/sjc433/val_total.parquet")
    print("val df saved")
    test_left.write.format("parquet").mode("overwrite").save("hdfs:/user/sjc433/test_total.parquet")
    print("test df saved")
